...
namespace internal {
  ...
  protected:
      ...
      PlanData & get_plan(int nfft,bool inverse,void * dst,const void * src)
      {
          ...
          int64_t key = ( (nfft<<3 ) | (size_t(inverse)<<2) | (size_t(inplace)<<1) | size_t(aligned) ) << 1;
          return m_plans[key];
      }
      ...
      PlanData & get_plan(int n0,int n1,bool inverse,void * dst,const void * src)
      {
          bool inplace = (dst==src);
          bool aligned = ( (reinterpret_cast<size_t>(src)&15) | (reinterpret_cast<size_t>(dst)&15) ) == 0;
          int64_t key = ( ( (((int64_t)n0) << 30)|(n1<<3 ) | (size_t(inverse)<<2) | (size_t(inplace)<<1) | size_t(aligned) ) << 1 ) + 1;
          return m_plans[key];
      }
  };
